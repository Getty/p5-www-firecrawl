...
=method endpoint_url(@path_parts)

Builds C<< <base_url>/<api_version>/<path_parts> >>.

=method parse_response($http_response)
...
=method scrape / scrape_request / parse_scrape_response

POST C</v2/scrape>. Returns the C<data> hash on success.

=method crawl / crawl_request / parse_crawl_response

POST C</v2/crawl>. Returns C<< { success, id, url } >>.

=method crawl_status / crawl_status_request / parse_crawl_status_response

GET C</v2/crawl/{id}>. Returns the full status object including
C<status>, C<total>, C<completed>, C<data>, and C<next> (pagination URL).

=method crawl_status_next($next_url)

Follow the C<next> URL verbatim for subsequent pages of a large crawl result.

=method crawl_cancel / crawl_cancel_request

DELETE C</v2/crawl/{id}>.

=method crawl_errors / crawl_errors_request

GET C</v2/crawl/{id}/errors>.

=method crawl_active / crawl_active_request

GET C</v2/crawl/active>.

=method crawl_params_preview / crawl_params_preview_request

POST C</v2/crawl/params/preview>.

=method map / map_request / parse_map_response

POST C</v2/map>. Returns the C<links> array.

=method search / search_request / parse_search_response

POST C</v2/search>.

=method batch_scrape / batch_scrape_request

POST C</v2/batch/scrape>. Returns C<< { id, url, invalidURLs } >>.

=method batch_scrape_status / batch_scrape_status_request

GET C</v2/batch/scrape/{id}>.

=method batch_scrape_status_next($next_url)

Follow pagination for batch-scrape results.

=method batch_scrape_cancel / batch_scrape_cancel_request

DELETE C</v2/batch/scrape/{id}>.

=method batch_scrape_errors / batch_scrape_errors_request

GET C</v2/batch/scrape/{id}/errors>.

=method extract / extract_request

POST C</v2/extract>.

=method extract_status / extract_status_request

GET C</v2/extract/{id}>.

=method agent / agent_status / agent_cancel

POST C</v2/agent>, GET/DELETE C</v2/agent/{id}>.

=method browser_create / browser_list / browser_delete / browser_execute

POST/GET/DELETE C</v2/browser/...>.

=method scrape_execute / scrape_browser_stop

Interactive scrape session endpoints.

=method credit_usage / credit_usage_historical / token_usage / token_usage_historical / queue_status / activity

GET monitoring endpoints.

=method scrape_many(\@urls, %scrape_opts)
...
=method retry_failed_pages(\%crawl_or_batch_result, %scrape_opts)
